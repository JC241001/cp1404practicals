"""
CP1404/CP5632 Practical
Recursion
"""


def do_it(n):
    """Do... it."""
    if n <= 0:
        return 0
    return n % 2 + do_it(n - 1)


# TODO: 1. write down what you think the output of this will be,

# output should be 3.  do_it works out how many odd numbers there are <= n


# TODO: 2. use the debugger to step through and see what's actually happening
#print(do_it(5))


def do_something(n):
    """Print the squares of positive numbers from n down to 0."""
    if n > 0:
        print(n ** 2)
        do_something(n - 1)

# TODO: 3. write down what you think the output of this will be,
# Should print the squares of the negative integers and not stop.

# TODO: 4. use the debugger to step through and see what's actually happening
do_something(4)
print()

# TODO: 5. fix do_something() so that it works the way it probably should :)

# change the < to > line25
# bring line 27 into if block


# Prints down, but with proper structure
def do_something_else(n):
    # BaseCase
    if n == 0:
        return

    print(n**2)
    do_something(n-1)

do_something_else(5)
print()


# Prints up (backwards)
def do_something_different(n):
    # BaseCase
    if n == 0:
        return

    do_something_different(n-1)
    print(n**2)

do_something_different(5)
